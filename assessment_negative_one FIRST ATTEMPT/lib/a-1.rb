...
def base2to10(binary)
  split = binary.split("")
  result = 0
  split.each_with_index do |num,idx|
    result+= (num.to_i * (2 ** idx))
  end
...
def lucas_numbers(n)
lucas_series = {-1 => -1 ,0 => 2, 1 => 1}
  if n == n.abs
    idx = 2
    while idx <= n
      lucas_series[idx]= lucas_series[idx-1] +  lucas_series[idx-2]
      idx+= 1
    end
    ...
    idx = -2
    while idx >= n
      lucas_series[idx]= lucas_series[idx+2] -  lucas_series[idx+2]
      idx-= 1
    end
  end
