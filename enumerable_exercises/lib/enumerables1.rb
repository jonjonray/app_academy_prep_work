...
def array_sum_with_index(arr)
  result = 0
  arr.each_with_index {|ele , idx| result = result + (ele * idx)}
  result
end
...
def price_is_right(bids, actual_retail_price)
  under_retail = bids.select {|ele| ele < actual_retail_price}
  return under_retail.max
end
...
def ordered_vowel_words(words)
  words.select {|ele| ordered_vowel_word?(ele)}
end
...
def ordered_vowel_word?(word)
  ...
  letters = word.split("")
  finder = letters.select {|ele| vowels.include? ele}

  return finder.sort == finder
...
def products_except_me(numbers)
  numbers.map.with_index do |ele,idx|
    int_array = numbers[0...idx] + numbers[idx+1..-1]
    array_product(int_array)
  end
end
